...
public class Fanout {
    ...
    private static void fanout2() throws InterruptedException {
        CompletableFuture<String> completableFuture = new CompletableFuture<>();
        completableFuture.thenApply(s -> {logger.info("1 {}", s);return s;}).thenAccept(s->logger.info("1 s{}", s));
        completableFuture.thenApply(s -> {logger.info("2 {}", s);return s;}).thenAccept(s->logger.info("2 s{}", s));
        completableFuture.thenApply(s -> {logger.info("3 {}", s);return s;}).thenAccept(s->logger.info("3 s{}", s));
        completableFuture.thenApply(s -> {logger.info("4 {}", s);return s;}).thenAccept(s->logger.info("4 s{}", s));
        completableFuture.complete("foo");
        Thread.sleep(100);
    }
}
